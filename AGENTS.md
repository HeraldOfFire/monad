...
- Built-in services (all opt-in, lazy): `db` (+`transaction`), `session`, `flash`, `csrf` (enforce via `verifyCsrf` middleware), `auth` (+`requireAuth` middleware), `cache`, `migrator`. Read `README.md` for the full API.
